...
def preprocess_flows(df: pd.DataFrame) -> List[FlowRecord]:
    ...
    working = working.sort_values(SORT_KEY_COLUMNS, kind="stable").reset_index(drop=True)
...
def make_train_sequences(
    ...
    random_indexes = [
        random.choices(range(len(bin_items)), k=samples_to_add)
        for bin_items in binnings
    ]
    for sample_idx in range(samples_to_add):
        seqs.append([bin_items[random_indexes[bin_idx][sample_idx]] for bin_idx, bin_items in enumerate(binnings)])
    return seqs
