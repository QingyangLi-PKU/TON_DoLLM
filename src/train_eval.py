...
def _load_records(
    ...
    """Load deterministic preprocessed records; construct FS only after seeding."""
    return (
        preprocess_flows(load_flow_csv(train_csv)),
        preprocess_flows(load_flow_csv(valid_csv)),
        preprocess_flows(load_flow_csv(current_detection_batch_csv)),
    )
...
def _build_loaders(
    ...
    records = _load_records(train_csv, valid_csv, current_detection_batch_csv)
    return _make_loaders(
        *records, num_flows, num_training_samples, batch_size, num_workers
    )
...
def run_all_experiments(
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    syn_path = Path(syn_dataset_dir)
    udp_path = Path(udp_dataset_dir)
    syn_records = _load_records(
        str(syn_path / "mixed_flows_train.csv"),
        str(syn_path / "mixed_flows_valid.csv"),
        str(syn_path / "mixed_flows_current_detection_batch.csv"),
    )
    udp_records = _load_records(
        str(udp_path / "mixed_flows_train.csv"),
        str(udp_path / "mixed_flows_valid.csv"),
        str(udp_path / "mixed_flows_current_detection_batch.csv"),
    )
    dataset_records = {"SYN": syn_records, "UDP": udp_records}
    rows: List[Dict[str, float]] = []

    for source_name, (train_records, valid_records, source_test_records) in dataset_records.items():
        target_name = "UDP" if source_name == "SYN" else "SYN"
        target_test_records = dataset_records[target_name][2]

        for seed in seeds:
            set_seed(int(seed))
            train_loader, valid_loader, source_test_loader, valid_size, source_test_size = _make_loaders(
                train_records,
                valid_records,
                source_test_records,
                num_flows,
            ...
            target_test_loader = DataLoader(
                FlowEvalDataset(target_test_records, num_flows=num_flows),
                batch_size=batch_size,
                shuffle=False,
                num_workers=num_workers,
            )
            ...
            for evaluated_name, loader, test_size, setting in (
                (source_name, source_test_loader, source_test_size, "in_domain"),
                (target_name, target_test_loader, len(target_test_records), "zero_shot"),
            ):
                metrics = evaluate_model(model, loader, test_size, device)
                rows.append(
                    {
                        "seed": int(seed),
                        "source_dataset": source_name,
                        "target_dataset": evaluated_name,
                        "setting": setting,
...
def run_zero_shot_experiment(
    ...
    num_epochs: int = 20,
    learning_rate: float = 1e-4,
    direction: str = "syn_to_udp",
) -> List[Dict[str, float]]:
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    if direction not in {"syn_to_udp", "udp_to_syn"}:
        raise ValueError("direction must be 'syn_to_udp' or 'udp_to_syn'")

    syn_path = Path(syn_dataset_dir)
    udp_path = Path(udp_dataset_dir)

    syn_train_records, syn_valid_records, syn_test_records = _load_records(
        str(syn_path / "mixed_flows_train.csv"),
        str(syn_path / "mixed_flows_valid.csv"),
        str(syn_path / "mixed_flows_current_detection_batch.csv"),
    )
    udp_train_records, udp_valid_records, udp_test_records = _load_records(
        str(udp_path / "mixed_flows_train.csv"),
        str(udp_path / "mixed_flows_valid.csv"),
        str(udp_path / "mixed_flows_current_detection_batch.csv"),
    )
    if direction == "syn_to_udp":
        source_name, target_name = "SYN", "UDP"
        source_train_records, source_valid_records, target_test_records = (
            syn_train_records,
            syn_valid_records,
            udp_test_records,
        )
    else:
        source_name, target_name = "UDP", "SYN"
        source_train_records, source_valid_records, target_test_records = (
            udp_train_records,
            udp_valid_records,
            syn_test_records,
    ...
    for seed in seeds:
        set_seed(int(seed))
        train_loader, valid_loader, test_loader, valid_size, test_size = _make_loaders(
        ...
        metrics = evaluate_model(model, test_loader, test_size, device)
        rows.append(
            {
                "seed": int(seed),
                "source_dataset": source_name,
    ...
    if not df.empty:
        summary = df[["f1", "accuracy", "precision", "recall", "auc"]].agg(["mean", "std"])
